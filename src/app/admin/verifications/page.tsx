...
import { 
  ShieldCheck, UserCheck, Search, 
  MapPin, Loader2, Trash2, Users, Mail, Droplets
} from "lucide-react";
...
export default function AdminVerifications() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case "receiver": return "bg-blue-100 text-blue-700 border-blue-200";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
          <div className="w-full lg:w-auto">
            ...
            <p className="text-gray-500 font-medium text-sm mt-1">Manage pending registration requests.</p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-gray-400 font-bold tracking-tight uppercase text-xs">Fetching data...</p>
          </div>
          ...
          <>
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-xs font-black uppercase text-gray-400">Info</th>
                    ...
                    <tr key={user._id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-black shrink-0 ${
                            user.role === 'admin' ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'
                          }`}>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col gap-0.5">
                           <span className="text-xs font-bold text-gray-700">{user.role !== 'admin' ? user.bloodGroup : 'N/A'}</span>
                           <span className="text-[10px] text-gray-400 flex items-center gap-1 uppercase tracking-tighter"><MapPin size={10}/>{user.city || "Global"}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => handleVerify(user._id)} className="p-2.5 bg-green-50 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all"><UserCheck size={18} /></button>
                          <DeleteDialog user={user} onDelete={() => handleDelete(user._id)} />
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
              {filteredUsers.map((user) => (
                <div key={user._id} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase border ${getRoleBadge(user.role)}`}>
                      {user.role}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 py-4 border-y border-gray-50 mb-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 uppercase font-black">Blood Type</span>
                      <span className="text-sm font-bold text-red-600 flex items-center gap-1"><Droplets size={14}/> {user.bloodGroup || "N/A"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 uppercase font-black">Location</span>
                      <span className="text-sm font-bold text-gray-700 flex items-center gap-1"><MapPin size={14}/> {user.city || "Global"}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      <DeleteDialog user={user} onDelete={() => handleDelete(user._id)} isMobile />
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white rounded-[2.5rem] p-16 text-center border border-dashed border-gray-200">
            ...
            <h3 className="text-lg font-black text-gray-800 uppercase tracking-tight">Queue Empty</h3>
            <p className="text-gray-400 text-sm mt-1">No pending accounts found in this category.</p>
          </div>
        )}
      </div>
    </div>
...
function DeleteDialog({ user, onDelete, isMobile = false }: { user: any, onDelete: () => void, isMobile?: boolean }) {
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <button className={`${isMobile ? 'w-full py-3 bg-red-50 text-red-600' : 'p-2.5 bg-red-50 text-red-600'} rounded-2xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-red-600 hover:text-white transition-all`}>
          <Trash2 size={18} /> {isMobile && "Reject"}
        </button>
      </AlertDialogTrigger>
      <AlertDialogContent className="rounded-[2rem] w-[90%] max-w-lg">
        <AlertDialogHeader>
          <AlertDialogTitle className="font-black text-xl">Confirm Rejection</AlertDialogTitle>
          <AlertDialogDescription className="text-gray-500 font-medium">
            Permanently delete <span className="text-red-600 font-bold">{user.username}</span>. This request will be removed from the system forever.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="flex flex-col md:flex-row gap-2 mt-4">
          <AlertDialogCancel className="rounded-xl font-bold md:mt-0">Wait, Go Back</AlertDialogCancel>
          <AlertDialogAction 
            onClick={onDelete}
